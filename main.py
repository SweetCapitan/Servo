...
# module_List_Choices = [str(file[:-3]) for file in os.listdir('modules') if file.endswith('.py')].append("all")


@slash_command(guild_ids=[os.environ.get("SERVER_ID")], name='reload',
               description='Перезагрузка модуля из памяти бота')
async def reload(ctx, module: Option(str,
                                     choices=module_list_choices, description="модуль, который будет перезапущен"), ):
    """ ХУЙ ПИЗДА СКОВРОДА"""
    ...
    if str(module) == 'all':
        ...
        return await ctx.respond(embed=re.done(f'Всего модулей перезагружено: {count}\n{module_list_text}'))
    else:
        bot.unload_extension(f'modules.{module}')
        bot.load_extension(f'modules.{module}')
        logger.comm(f'RELOAD module: [{module}]. Author: {ctx.author}')
        return await ctx.respond(embed=re.done(f'Модуль [{module}] перезагружен'))


@slash_command(guild_ids=[os.environ.get("SERVER_ID")], name='load',
